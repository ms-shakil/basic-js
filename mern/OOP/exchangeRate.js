...
// static
class ExchangeRate2 {
      static apiUrl ="https://api.exchangerate-api.com/v4/latest/"
     static async getRate(c1='usd',c2){
          const url = this.apiUrl+c1
          const response = await fetch(url)
          const data = await response.json()
          const rate = data.rates[c2.toUpperCase()]
...
(async () => {
  ...
  const value = await ExchangeRate2.getRate("usd", "bdt") // static use korle object er moto call korajai
  console.log(value)
})()